import{l as $,c as H,C as N,M as q}from"./hotspots-IBT_bgf8.js";import{c as z,V as _,a as Q,b as K,d as W,e as X,f as j,g as J,h as Y,i as Z}from"./labUi-CJ4rbYiC.js";import{g as tt}from"./runtimeConfig-bT2k2tCC.js";import{d as at,c as et}from"./index-B6wQMjJX.js";function ot(){const o=document.createElement("section");return o.className="mode-page lab-mode-page immersive-master-page vr-mode-page",o.innerHTML=`
  ...
  `,o}function a(o,s){return o.querySelector(s)}function B(o,s,t){const n=Math.round(q.clamp(s,0,1)*100);a(o,"[data-loading-bar]").style.width=`${n}%`,a(o,"[data-loading-detail]").textContent=t||(n<98?`正在加载沉浸实验室 · ${n}%`:"正在校准第一人称光路…")}function G(o,{mode:s,label:t,index:n,count:w,progress:x}){a(o,"[data-tour-mode]").textContent=s,a(o,"[data-tour-step]").textContent=t,a(o,"[data-tour-index]").textContent=`${String(n+1).padStart(2,"0")} / ${String(w).padStart(2,"0")}`,a(o,"[data-tour-line]").style.width=`${Math.round(x*100)}%`}function D(o,s){console.error("VR mode failed to start.",s);const t=a(o,"[data-loading]");t.classList.add("is-error"),t.setAttribute("aria-busy","false"),t.querySelector("h1").textContent="沉浸实验室加载失败",a(o,"[data-loading-detail]").textContent="请确认模型资源存在，并通过本地服务打开页面。",a(o,"[data-retry]").hidden=!1}async function rt(o,{navigate:s}){const t=ot();o.append(t);const n=et(),w=a(t,".lab-scene-canvas"),x={interface:a(t,"[data-interface]"),entryTransition:a(t,"[data-entry-transition]"),entryTitle:a(t,"[data-entry-title]"),entryDetail:a(t,"[data-entry-detail]"),toast:a(t,"[data-toast]"),spatialPanel:a(t,"[data-spatial-panel]"),spatialKicker:a(t,"[data-spatial-kicker]"),spatialTitle:a(t,"[data-spatial-title]"),spatialBody:a(t,"[data-spatial-body]"),crosshair:a(t,"[data-focus-reticle]"),orientationHint:a(t,"[data-orientation-hint]"),returnTour:a(t,"[data-return-tour]")},r=Y(x),k=tt();t.dataset.runtimeLevel=k.level.toLowerCase(),n.add(()=>r.dispose());let l;try{l=z(w,{modeConfig:_,runtimeConfig:k})}catch(e){return D(t,e),{dispose:()=>n.dispose()}}const{scene:f,camera:p,controls:E,renderer:m}=l,I=Z(p);let h=0;n.add(()=>{I.disable(),m.xr&&(m.xr.enabled=!1),m.setAnimationLoop(null),l.dispose(),at(f),t.remove(),document.body.removeAttribute("data-view-mode"),delete document.body.dataset.climax}),a(t,"[data-back]").addEventListener("click",()=>s("/home")),a(t,"[data-retry]").addEventListener("click",()=>window.location.reload());let i;try{i=await $({onProgress:e=>B(t,e),onSourceChange:()=>B(t,.1,"压缩模型不可用，正在切换兼容版本…")})}catch(e){return D(t,e),{dispose:()=>n.dispose()}}f.add(i.root),i.applyVisualConfig?.(_.model),l.setGridFloor(i.bounds.min.y-.035);const M=Q({scene:f,camera:p,anchors:i.anchors,isMobile:l.isMobile,reducedMotion:l.reducedMotion,visualConfig:_.effects,runtimeConfig:k}),v=H({camera:p,anchors:i.anchors,layer:a(t,"[data-hotspots]"),card:a(t,"[data-hotspot-card]"),closeButton:a(t,"[data-hotspot-close]")});n.add(()=>v.dispose());const g=K({scene:f,camera:p,hotspots:v,anchors:i.anchors,ui:r});n.add(()=>g.dispose());const C=W({element:a(t,"[data-climax]"),valueElement:a(t,"[data-climax-value]"),onTick:(e,d)=>{d===2&&i.triggerGlow(.72)},onImpact:()=>{M.triggerClimax(h),i.triggerGlow(1.8),document.body.dataset.climax="complete",r.setSpatialPanel({kicker:"S = 2.52",title:"突破经典上限 S ≤ 2",body:"符合计数显示贝尔不等式被违反，量子关联不能用经典局域模型解释。"})}});n.add(()=>C.dispose());const u=X({camera:p,controls:E,anchors:i.anchors,bounds:i.bounds,modeLabel:"第一人称导览",firstPerson:!0,onUpdate:e=>{G(t,e),["scan","motion"].includes(document.body.dataset.viewMode)&&r.setSpatialPanel({kicker:e.mode,title:e.label,body:"可切换体感或自由查看；不支持 WebXR 也不会影响当前画面。"})},onPause:e=>{G(t,{mode:e,label:"可用手指拖动查看；体感需要点击按钮授权。",index:0,count:1,progress:1})},onClimax:()=>C.play()});n.add(()=>u.dispose());const F=j({sceneSystem:l,effects:M,dot:a(t,"[data-performance-dot]"),label:a(t,"[data-performance]"),runtimeConfig:k}),c=J({app:t,camera:p,orbitControls:E,anchors:i.anchors,bounds:i.bounds,gyroControls:I,showToast:r.showToast,onEnterTransition:r.playEntranceTransition,onModeChange:e=>{r.setMode(e);const d=e==="scan"||e==="motion";v.setEnabled(!d),g.setEnabled(d),e!=="motion"&&T(!1)}});n.add(()=>c.dispose());async function S(){c.setTourMode(),r.setMode("tour"),v.setEnabled(!0),g.setEnabled(!1),C.reset(),document.body.dataset.climax="pending",i.mixer.setTime(0),r.playEntranceTransition("第一人称导览开始","从实验室入口推进到纠缠光路"),u.restart(h),r.showToast("自动导览已开始，可以随时拖动打断。")}function T(e){a(t,"[data-motion-toggle]").textContent=e?"关闭体感":"手机体感模式";const d=a(t,"[data-mobile-motion-toggle]");d&&(d.textContent=e?"关闭体感":"手机体感")}function L(){const e=a(t,"[data-mobile-vr-start]");e&&(e.hidden=!0)}function V(e){const d=a(t,"[data-mobile-drawer]"),b=a(t,"[data-mobile-drawer-backdrop]"),U=a(t,"[data-mobile-menu]");d.hidden=!e,b.hidden=!e,U?.setAttribute("aria-expanded",String(e))}function y(){V(!1)}async function A(){L(),u.free(),await c.enterScanMode({fullscreen:!0})}async function P(){if(L(),c.motionActive){c.disableMotion(),T(!1);return}u.free();const e=await c.enableMotion();T(e)}async function R(){L(),u.free(),await c.exitToFree({exitScreen:!0}),v.setEnabled(!0),g.setEnabled(!1)}a(t,"[data-tour-restart]").addEventListener("click",S),a(t,"[data-phone]").addEventListener("click",A),a(t,"[data-motion-toggle]").addEventListener("click",P),a(t,"[data-tour-free]").addEventListener("click",R),a(t,"[data-return-tour]").addEventListener("click",S),a(t,"[data-mobile-back]").addEventListener("click",()=>s("/home")),a(t,"[data-mobile-home]").addEventListener("click",()=>s("/home")),a(t,"[data-mobile-vr-home]").addEventListener("click",()=>s("/home")),a(t,"[data-mobile-menu]").addEventListener("click",()=>{const e=a(t,"[data-mobile-drawer]");V(e.hidden)}),a(t,"[data-mobile-close]").addEventListener("click",y),a(t,"[data-mobile-drawer-backdrop]").addEventListener("click",y),a(t,"[data-mobile-vr-phone]").addEventListener("click",A),a(t,"[data-mobile-vr-motion]").addEventListener("click",P),a(t,"[data-mobile-vr-free]").addEventListener("click",R),a(t,"[data-mobile-tour-restart]").addEventListener("click",()=>{y(),L(),S()}),a(t,"[data-mobile-phone]").addEventListener("click",async()=>{y(),await A()}),a(t,"[data-mobile-motion-toggle]").addEventListener("click",async()=>{y(),await P()}),a(t,"[data-mobile-free]").addEventListener("click",async()=>{y(),await R()}),a(t,"[data-mobile-ar]").addEventListener("click",()=>s("/ar"));const O=new N;return m.setAnimationLoop((e,d)=>{const b=Math.min(O.getDelta(),.05);h+=b,i.update(b),u.update(h),c.update(b),!u.active&&E.enabled&&E.update(),M.update(b,h,i.animationTime),v.update(),g.update(b),m.xr?.isPresenting&&d?m.render(f,p):l.render(),F.update(b,{immersiveActive:["scan","motion"].includes(c.mode),isMobile:l.isMobile})}),u.restart(h),window.requestAnimationFrame(()=>{a(t,"[data-interface]").classList.add("is-ready"),a(t,"[data-interface]").setAttribute("aria-hidden","false"),a(t,"[data-loading]").setAttribute("aria-busy","false"),a(t,"[data-loading]").classList.add("is-done")}),document.body.dataset.climax="pending",{dispose:()=>n.dispose()}}export{rt as mount};
